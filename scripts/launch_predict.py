"""Launch an nnU-Net v2 prediction experiment from a run folder."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Launch an inference experiment from an nnU-Net autoresearch run folder."
    )
    parser.add_argument(
        "--exp",
        required=True,
...
def resolve_prediction_paths(run_dir: Path, dataset_config: dict[str, Any]) -> dict[str, str]:
    """Resolve required input and output paths for prediction."""
    paths = require_mapping(dataset_config, "paths", "dataset config field")
    predict_input_root = require_text(paths, "predict_input_root", "dataset paths field")
    require_text(paths, "predict_output_root", "dataset paths field")

    input_dir = Path(predict_input_root).expanduser()
    if not input_dir.exists() or not input_dir.is_dir():
        raise FileNotFoundError(f"Prediction input directory does not exist: {input_dir}")

    output_dir = run_dir / "artifacts" / "predictions"
...
def resolve_model_dir(
    ...
    """Resolve a conservative common nnU-Net model directory."""
    results_root = Path(nnunet_results)
    checked_dirs: list[Path] = []
    for path in (
        results_root / dataset_id / f"{trainer}__{default_plan}__{configuration}",
        results_root / dataset_id / f"{trainer}__nnUNetPlans__{configuration}",
    ):
    ...
    raise FileNotFoundError(
        "Could not resolve nnU-Net model directory. Checked: " + checked_text
    )


def validate_inference_context(
    meta: dict[str, Any], global_config: dict[str, Any], dataset_config: dict[str, Any]
) -> dict[str, str]:
    """Validate inference metadata and runtime config and return resolved values."""
    if str(meta.get("task_type", "")).strip() != "inference":
        raise ValueError("launch_predict.py only supports experiments with task_type=inference")

    dataset_key = require_text(meta, "dataset_key", "meta.yaml field")
    inputs = require_mapping(meta, "inputs", "meta.yaml field")
    execution = require_mapping(meta, "execution", "meta.yaml field")
    ...
    checkpoint = require_text(inputs, "checkpoint", "inputs field")
    ...
    default_plan = str(dataset_config.get("default_plan", "nnUNetPlans")).strip() or "nnUNetPlans"
...
def build_command(resolved: dict[str, str]) -> str:
    ...
    return " ".join(
        [
            ...
            "nnUNetv2_predict",
            "-i",
            shell_quote(resolved["input_dir"]),
            "-o",
            shell_quote(resolved["output_dir"]),
            "-d",
            ...
            "-chk",
            shell_quote(resolved["checkpoint"]),
        ]
    )
...
def ensure_bash_available() -> str:
    ...
    if not bash_path:
        raise RuntimeError(
            "bash is required to launch prediction commands because command.sh is Linux-oriented."
        )
    return bash_path
...
def launch_with_tmux(
    ...
    """Launch the prediction command in a detached tmux session."""
    tmux_command = (
...
def launch_in_foreground(
    ...
    """Launch the prediction command in the current shell and block until it exits."""
...
def main() -> int:
    """Resolve, validate, and launch an nnU-Net prediction command."""
    args = parse_args()
    root = project_root()
    ...
    resolved = validate_inference_context(meta, global_config, dataset_config)
    resolved.update(resolve_prediction_paths(run_dir, dataset_config))
    model_dir = resolve_model_dir(
        resolved["nnUNet_results"],
        resolved["dataset_id"],
        resolved["trainer"],
